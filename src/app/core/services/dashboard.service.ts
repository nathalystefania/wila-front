...
export class DashboardService {
    ...
    private construirDashboard(
        ...
        const motorIdPorAnilloId =
            this.crearMotorPorAnillo(
                anillos
            );

        const motorIdPorCarbonId =
            this.crearMotorPorCarbon(
                carbones,
                motorIdPorAnilloId
            );

        const carbonIdsPorMotorId =
            this.agruparCarbonesPorMotor(
                motorIdPorCarbonId
            );

        const ultimaLecturaPorCarbonId =
            this.obtenerUltimaLecturaPorCarbon(
                telemetria
            );
        ...
        const alarmasEmpresa =
            alarmas.filter(alarma =>
                idsCarbonesEmpresa.has(
                    this.normalizarId(
                        alarma.carbon_id
                    )
                )
        ...
        const totalAlarmasP2 =
            alarmasEmpresa.filter(
                alarma =>
                    this.normalizarSeveridad(
                        alarma.severidad
                    ) === 'critica'
            ).length;

        const totalAlarmasP1 =
            alarmasEmpresa.filter(
                alarma =>
                    this.normalizarSeveridad(
                        alarma.severidad
        ...
        const filasMotores:
            ...
            motoresConfigurados
                .map(motor => {
                    ...
                    const carbonIdsMotor =
                        carbonIdsPorMotorId.get(
                            motorId
                        ) ?? new Set<string>();

                    const lecturasMotor =
                        this.obtenerLecturasMotor(
                            carbonIdsMotor,
                            ultimaLecturaPorCarbonId
                        );
