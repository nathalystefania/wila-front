...
export class MotoresService {
    ...
    createMotor(_plantaId: number, motor: MotorDraft) {
        return of<CreateMotorResponse>({ id: ++mockMotorIdCounter });
    }

    async checkMotoresExist(_plantaId: number): Promise<boolean> {
        return true;
    }

    async createMotores(_plantaId: number, motores: MotorDraft[]) {
    ...
    async createCarbonesForAnillo(anillo: Anillo, cantidadCarbones: number, _motorDraft: MotorDraft): Promise<number[]> {
        return Array.from({ length: cantidadCarbones }, (_, i) => ++mockCarbonIdCounter);
    }

    getMotoresByPlanta(_plantaId: number) {
    ...
    updateMotor(_motorId: number, motor: MotorDraft) {
    ...
    async createOrUpdateMotores(_plantaId: number, motores: MotorDraft[], existingIds: number[] = []): Promise<number[]> {
